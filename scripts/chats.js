    ...
    Chat.Messages.forEach(async Message => {
        ...
        MessagePart.querySelector("span").querySelectorAll("img").forEach(ImageNode => {
            ...
            ImageNode.addEventListener("mouseenter", () => {
                document.querySelectorAll(`body *:not(.Media):not(.Media *):not(input):not(*:has(input)):not(*:has(.${Class}))`).forEach(Node => {
                    if (!Node.isSameNode(ImageNode)) {
                        Node.style.filter = "blur(2px)";
                    }
                });
                document.querySelectorAll(`body .Media *, .${Class} *:has(.${Class})`).forEach(Node => (Node.style.filter = "none"));
            });
        ...
        // If the message is from the current user, add the remove button
...
async function GenerateChat(Chat) {
    ...
    await new Api.Storage("Users").GetDocumentsByField("Username", Chat.Users.filter((User) => User !== CurrentUser).join(", ")).then((Document) => {
        ...
        ChatPart.innerHTML = ChatPart.innerHTML + `<img src="${Document[0].ProfileImage || "../images/Default.svg"}"><span>${Chat.Users.filter((User) => User !== CurrentUser).join(", ")}</span><span style="opacity: 0;">#${Document[0].Tag}</span>`;
    });
