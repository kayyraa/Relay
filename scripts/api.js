...
export class VoiceChat {
    ...
    ListenToOtherUserStream(VoiceChatId) {
        ...
        VoiceChatStorage.GetDocument(VoiceChatId).then((Document) => {
            if (Document && Document[0] && Document[0].Data) {
                ...
                if (Offer && this.Peer && !this.Peer.destroyed) {
                    this.Peer.signal(Offer);
                } else {
                    console.error("Peer is destroyed or no offer found.");
                }
            }
        });
    }    
    ...
    async Setup() {
        ...
        this.Peer.on("signal", async (Data) => {
            if (this.Peer.destroyed) {
                console.error("Peer connection destroyed. Cannot signal.");
                return;
            }
            ...
            if (this.Peer.signalReady) {
                this.Peer.signal(Data); // Ensure peer is ready to signal
            } else {
                console.error("Peer not ready for signaling");
            }
        });              
    ...
    async Disconnect() {
        ...
        if (this.Peer) {
            this.Peer.destroy();  // Ensure the peer is destroyed
        }
    
        if (this.LocalStream) {
            this.LocalStream.getTracks().forEach(Track => Track.stop());  // Stop the media tracks
        }
